...
Schema.prototype.clone = function() {
  ...
  s._originalSchema =
    this._originalSchema == null ? this._originalSchema : this._originalSchema.clone();
...
reserved['prototype'] =
  ...
  reserved.emit = reserved.on = reserved.once = reserved.listeners = reserved.removeListener =
    ...
    reserved.collection = reserved.db = reserved.errors = reserved.init = reserved.isModified = reserved.isNew = reserved.get = reserved.modelName = reserved.save = reserved.schema = reserved.toObject = reserved.validate = reserved.remove = reserved.populated =
      // hooks.js
...
Schema.prototype.path = function(path, obj) {
  ...
  if (schemaType.$isSchemaMap) {
    ...
    const mapPath = path + '.$*';
    this.paths[path + '.$*'] = this.interpretAsType(mapPath, obj.of || { type: {} }, this.options);
...
Schema.prototype.interpretAsType = function(path, obj, options) {
  ...
  let type =
    obj[options.typeKey] && (options.typeKey !== 'type' || !obj.type.type)
      ? obj[options.typeKey]
      : {};
  let name;
  ...
  if (Array.isArray(type) || Array === type || type === 'array') {
    ...
    if (typeof cast === 'string') {
      ...
      cast &&
      (!cast[options.typeKey] || (options.typeKey === 'type' && cast.type.type)) &&
      utils.isPOJO(cast)
    ) {
...
Schema.prototype.setupTimestamp = function(timestamps) {
  ...
  this.pre('save', function(next) {
    ...
    const defaultTimestamp = (this.ownerDocument
      ? this.ownerDocument()
      : this
        ...
        if (foundschema.caster) {
          ...
          if (p !== parts.length && foundschema.schema) {
            ...
            if (parts[p] === '$' || isArrayFilter(parts[p])) {
              ...
              if (ret) {
                ret.$isUnderneathDocArray =
                  ret.$isUnderneathDocArray || !foundschema.schema.$isSingleNested;
              }
              return ret;
            }
