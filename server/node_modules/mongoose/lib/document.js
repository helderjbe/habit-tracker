...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  try {
    ...
    if (
      ...
      (schema.options[this.schema.options.typeKey][0].ref === val[0].constructor.baseModelName ||
        schema.options[this.schema.options.typeKey][0].ref === val[0].constructor.modelName)
    ) {
      if (this.ownerDocument) {
        popOpts = { model: val[0].constructor };
        this.ownerDocument().populated(
          this.$__fullPath(path),
          val.map(function(v) {
            return v._id;
          }),
          popOpts
...
Document.prototype.$__validate = function(options, callback) {
  ...
  const hasValidateModifiedOnlyOption =
    options && typeof options === 'object' && 'validateModifiedOnly' in options;
  ...
  if (paths.length === 0) {
    return process.nextTick(function() {
      ...
      if (error) {
        return _this.schema.s.hooks.execPost(
          'validate:error',
          _this,
          [_this],
          { error: error },
  ...
  const validatePath = function(path) {
    ...
    process.nextTick(function() {
      ...
      p.doValidate(
        ...
        scope,
        { skipSchemaValidators: skipSchemaValidators[path], path: path }
      );
    });
  };
...
Document.prototype.$__dirty = function() {
  ...
  all = all.concat(
    this.$__.activePaths.map('default', function(path) {
