...
function Model(doc, fields, skipId) {
  if (fields instanceof Schema) {
    throw new TypeError(
      '2nd argument to `Model` must be a POJO or string, ' +
...
Model.prototype.$__save = function(options, callback) {
  this.$__handleSave(options, (error, result) => {
    if (error) {
      return this.schema.s.hooks.execPost('save:error', this, [this], { error: error }, function(
        error
      ) {
        callback(error);
      });
...
Model.prototype.save = function(options, fn) {
  ...
  return utils.promiseOrCallback(
    fn,
    cb => {
      if (parallelSave) {
...
Model.init = function init(callback) {
  ...
  const autoIndex =
    this.schema.options.autoIndex == null
      ? this.db.config.autoIndex
      : this.schema.options.autoIndex;
  const autoCreate =
    this.schema.options.autoCreate == null
...
Model.createCollection = function createCollection(options, callback) {
  if (typeof options === 'string') {
    throw new Error(
      "You can't specify a new collection name in Model.createCollection." +
    ...
    this.listIndexes((err, indexes) => {
      ...
      for (const index of indexes) {
        ...
        for (const schemaIndex of schemaIndexes) {
          ...
          const options = _decorateDiscriminatorIndexOptions(this, utils.clone(schemaIndex[1]));
          ...
          if (utils.deepEqual(key, index.key) && utils.deepEqual(index, indexCopy)) {
            found = true;
            break;
          }
        }
...
function _ensureIndexes(model, options, callback) {
  ...
  const create = function() {
    ...
    model.collection[methodName](
      indexFields,
      indexOptions,
      utils.tick(function(err, name) {
...
function _decorateDiscriminatorIndexOptions(model, indexOptions) {
  ...
  if (
    model.baseModelName != null &&
    indexOptions.unique &&
    !('partialFilterExpression' in indexOptions) &&
    !('sparse' in indexOptions)
  ) {
    const value =
      (model.schema.discriminatorMapping && model.schema.discriminatorMapping.value) ||
      model.modelName;
...
const safeDeprecationWarning =
  'Mongoose: the `safe` option for `save()` is ' +
...
Model.findOneAndUpdate = function(conditions, update, options, callback) {
  if (typeof options === 'function') {
    ...
    if (typeof conditions === 'function') {
      const msg =
        'Model.findOneAndUpdate(): First argument must not be a function.\n\n' +
        '  ' +
        this.modelName +
...
Model.findByIdAndUpdate = function(id, update, options, callback) {
  ...
  if (arguments.length === 1) {
    if (typeof id === 'function') {
      const msg =
        'Model.findByIdAndUpdate(): First argument must not be a function.\n\n' +
        '  ' +
        this.modelName +
...
Model.findOneAndDelete = function(conditions, options, callback) {
  if (arguments.length === 1 && typeof conditions === 'function') {
    const msg =
      'Model.findOneAndDelete(): First argument must not be a function.\n\n' +
      '  ' +
      this.modelName +
...
Model.findByIdAndDelete = function(id, options, callback) {
  if (arguments.length === 1 && typeof id === 'function') {
    const msg =
      'Model.findByIdAndDelete(): First argument must not be a function.\n\n' +
      '  ' +
      this.modelName +
...
Model.findOneAndReplace = function(filter, replacement, options, callback) {
  if (arguments.length === 1 && typeof filter === 'function') {
    const msg =
      'Model.findOneAndReplace(): First argument must not be a function.\n\n' +
      '  ' +
      this.modelName +
      '.findOneAndReplace(conditions, callback)\n' +
      '  ' +
      this.modelName +
      '.findOneAndReplace(conditions)\n' +
      '  ' +
      this.modelName +
      '.findOneAndReplace()\n';
    throw new TypeError(msg);
...
Model.findOneAndRemove = function(conditions, options, callback) {
  if (arguments.length === 1 && typeof conditions === 'function') {
    const msg =
      'Model.findOneAndRemove(): First argument must not be a function.\n\n' +
      '  ' +
      this.modelName +
...
Model.findByIdAndRemove = function(id, options, callback) {
  if (arguments.length === 1 && typeof id === 'function') {
    const msg =
      'Model.findByIdAndRemove(): First argument must not be a function.\n\n' +
      '  ' +
      this.modelName +
          ...
          const Model =
            this.discriminators && doc[discriminatorKey] != null
              ? this.discriminators[doc[discriminatorKey]] ||
                getDiscriminatorByValue(this, doc[discriminatorKey])
              : this;
          if (Model == null) {
            throw new Error(
...
function getModelsMapForPopulate(model, docs, options) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (virtual && virtual.options) {
      const virtualPrefix = virtual.$nestedSchemaPath ? virtual.$nestedSchemaPath + '.' : '';
    ...
    while (k--) {
      ...
      try {
        Model =
          originalModel && originalModel[modelSymbol]
            ? originalModel
  ...
  function _getModelNames(doc, schema) {
    ...
    if (modelNameFromQuery) {
      ...
      if (
        options._queryProjection != null &&
        isPathExcluded(options._queryProjection, normalizedRefPath)
      ) {
        throw new Error(
          'refPath `' +
            normalizedRefPath +
...
Model.__subclass = function subclass(conn, schema, collection) {
  ...
  if (!collection) {
    collection =
      _this.prototype.schema.get('collection') ||
