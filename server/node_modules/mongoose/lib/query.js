...
Query.prototype.select = function select() {
  ...
  if (
    (('string' == type || Object.prototype.toString.call(arg) === '[object Arguments]') &&
...
const safeDeprecationWarning =
  'Mongoose: the `safe` option is deprecated. ' +
...
Query.prototype._findOneAndDelete = wrapThunk(function(callback) {
  ...
  this._collection.collection.findOneAndDelete(
    filter,
    options,
    _wrapThunkCallback(this, (err, res) => {
      if (err) {
        return callback(err);
      }
  ...
  if (useFindAndModify === false) {
    ...
    if (type === 'remove') {
      collection.findOneAndDelete(
        castedQuery,
        opts,
        _wrapThunkCallback(_this, function(error, res) {
...
function _updateThunk(op, callback) {
  ...
  if (isOverwriting) {
    if (op === 'updateOne' || op === 'updateMany') {
      return callback(
        new MongooseError(
          'The MongoDB server disallows ' +
            'overwriting documents using `' +
            op +
            '`. See: ' +
  ...
  try {
    return cast(
      ...
      obj,
      {
        ...
        strictQuery:
          (this.options && this.options.strictQuery) ||
