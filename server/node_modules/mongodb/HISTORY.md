...
- Updated mongodb-core to 2.1.9.
  ...
  _ NODE-957 Fixed issue where < batchSize not causing cursor to be closed on execution of first batch. \* NODE-958 Store reconnectConnection on pool object to allow destroy to close immediately.
...
- Moved BSON*BINARY_SUBTYPE_DEFAULT from BSON object to Binary object and removed the BSON_BINARY* prefixes
...
- Allow for force send query to primary, pass option (read:'primary') on find command. \* `find({a:1}, {read:'primary'}).toArray(function(err, items) {});`
...
- Eval function no accepts options including nolock (Issue #432) \* eval(code, parameters, options, callback) (where options = {nolock:true})
